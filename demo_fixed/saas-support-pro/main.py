"""SaaS Support Pro — AI Agency powered by Forge Runtime."""
...
logger = logging.getLogger("saas-support-pro")
...
def build_agency() -> tuple[Agency, EventLog]:
    """Build and configure the SaaS Support Pro agency."""
    ...
    support_team_agents = [
        Agent(
            ...
            temperature=0.3,
            domain_knowledge=domain_knowledge if _has_primitives else None,
            tools=[t for t in [_builtin_tools.get("query_database"), _builtin_tools.get("http_request"), ] if t is not None],
        ),
        Agent(
            ...
            temperature=0.3,
            domain_knowledge=domain_knowledge if _has_primitives else None,
            tools=[t for t in [_builtin_tools.get("query_database"), ] if t is not None],
        ),
        Agent(
            ...
            temperature=0.6,
            domain_knowledge=domain_knowledge if _has_primitives else None,
            tools=[t for t in [_builtin_tools.get("send_email"), ] if t is not None],
        ),
    ]

    support_team_team = Team(
        name="Support Team",
        lead=support_team_lead,
        agents=support_team_agents,
        shared_memory=memory,
    )
    agency.add_team(support_team_team)
...
Tool Usage: You have access to real tools. Use score_lead to evaluate real prospects. Use add_to_nurture_sequence to enroll leads in campaigns. Use get_pipeline_metrics to track real pipeline data. Always use your tools to execute, not just plan.""",
    ...
    # ═══════════════════════════════════════════════════════════
    memory.store("company:name", "SaaS Support Pro", author="system", tags=["config"])
